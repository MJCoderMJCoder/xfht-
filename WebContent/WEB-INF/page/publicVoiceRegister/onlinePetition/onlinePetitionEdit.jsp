...
<script type="text/javascript">
	...
	function save(){
		...
		var matterDetailed = $("#matterDetailed").val();
		if(matterDetailed.trim() == ""){
			$("#warningNotice").html("问题细类不能为空！");
			$(".ts_block").css("display","block");
			return false;
		}
		
		var reviewOpinions = $('input:radio[name="reviewOpinions"]:checked').val();
		if(reviewOpinions == null){
			$("#warningNotice").html("是否有复核意见书不能为空！");
			$(".ts_block").css("display","block");
			return false;
		}
		...
		if(accept == null){
			$("#warningNotice").html("法院是否已受理不能为空！");
			$(".ts_block").css("display","block");
			return false;
		}
		
		var administrativeReconsideration = $('input:radio[name="administrativeReconsideration"]:checked').val();
		if(administrativeReconsideration == null){
			$("#warningNotice").html("是否已经行政复议不能为空！");
			$(".ts_block").css("display","block");
			return false;
		}
		
		var publicity = $('input:radio[name="publicity"]:checked').val();
		if(publicity == null){
			$("#warningNotice").html("是否允许公开不能为空！");
			$(".ts_block").css("display","block");
			return false;
		}
									...
									<tbody>
										<tr>
											...
											<td width="30%" colspan="2">
												<input class="input_color" id="complaintNumber" name="complaintNumber" type="text" placeholder="请输入信访编号" maxlength="50" value="${guestbook.complaintNumber}" readonly="readonly"/>
											</td>
											<td width="20%" class="table_grey">创建日期：</td>
											<td width="30%" colspan="2">
												<input class="input_color" id="createTime" name="createTime" type="text" placeholder="请输入创建日期" maxlength="50" value="${guestbook.createTime}" readonly="readonly"/>
											</td>
										</tr>
										<tr>
											<td width="20%" class="table_grey">姓　　名：</td>
											<td width="30%" colspan="2">
												<input class="input_color" id="informantName" name="informantName" value="${guestbook.ext.informantName}" type="text" placeholder="请输入姓名" maxlength="10" />
											</td>
											<td class="table_grey">身份证号：</td>
											<td colspan="2">
												<input class="input_color" id="informantCardNo" name="informantCardNo" value="${guestbook.ext.informantCardNo}" type="text" placeholder="请输入身份证号" maxlength="18" />
											</td>
										</tr>
										<tr>
											<td class="table_grey">级　　别：</td>
											<td colspan="2">
												<select id="informantLevel" name="informantLevel">
													<option value=""></option>
													<c:forEach items="${levelList }" var="obj" varStatus="status">
														<option value="${obj.name }" ${guestbook.ext.informantLevel==obj.name?'selected':''}>${obj.name }</option>
													</c:forEach>
												</select>
											</td>
											...
											<td colspan="2">
												<select id="informantpoliticalFace" name="informantpoliticalFace">
													...
													<c:forEach items="${politicalFaceList }" var="obj" varStatus="status">
														<option value="${obj.name }" ${guestbook.ext.informantpoliticalFace==obj.name?'selected':''}>${obj.name }</option>
													</c:forEach>
												</select>
											</td>
										</tr>
										<tr>
											...
											<td colspan="2">
												<input class="input_color" id="informantPhone" name="informantPhone" value="${guestbook.ext.informantPhone}" type="text" placeholder="请输入手机号" maxlength="11" />
											</td>
											<td class="table_grey">居住地址：</td>
											<td colspan="2">
												<input class="input_color" id="informantPlace" name="informantPlace" value="${guestbook.ext.informantPlace}" type="text" placeholder="请输入居住地址" maxlength="50" />
											</td>
										</tr>
										<tr>
											<td class="table_grey"><span>*</span>标　　题：</td>
											<td colspan="2">
												<input class="input_color" id="title" name="title" value="${guestbook.ext.title}" type="text" placeholder="请输入标题" maxlength="50" />
											</td>
											...
											<td colspan="2">
												<select id="matterCategory" name="matterCategory">
													...
													<c:forEach items="${matterCategoryList }" var="obj" varStatus="status">
														<option value="${obj.name }" ${guestbook.ext.matterCategory==obj.name?'selected':''}>${obj.name }</option>
													</c:forEach>
											</select></td>
										</tr>
										<tr>
											<td class="table_grey"><span>*</span>问题细类：</td>
											<td colspan="2">
												<input class="input_color" id="matterDetailed" name="matterDetailed" value="${guestbook.ext.matterDetailed}" type="text" placeholder="请输入问题细类" maxlength="50" />
											</td>
											<td width="20%" class="table_grey"><span>*</span>是否有复核意见书：</td>
											<td colspan="2">
												<c:choose>
													<c:when test="${guestbook.ext.reviewOpinions}">
														<input type="radio" id="reviewOpinions" name="reviewOpinions" value="1" class="sex" checked>是 
														<input type="radio" id="reviewOpinions" name="reviewOpinions" value="0" class="sex">否
													</c:when>
													<c:otherwise>
														<input type="radio" id="reviewOpinions" name="reviewOpinions" value="1" class="sex">是 
														<input type="radio" id="reviewOpinions" name="reviewOpinions" value="0" class="sex" checked>否
													</c:otherwise>
												</c:choose>
											</td>
										</tr>
										<tr>
											<td width="20%" class="table_grey"><span>*</span>法院是否已受理：</td>
											<td colspan="2">
												<c:choose>
													<c:when test="${guestbook.ext.accept}">
														<input type="radio" id="accept" name="accept" value="1" class="sex" checked>是 
														<input type="radio" id="accept" name="accept" value="0" class="sex">否
													</c:when>
													<c:otherwise>
														<input type="radio" id="accept" name="accept" value="1" class="sex">是 
														<input type="radio" id="accept" name="accept" value="0" class="sex" checked>否
													</c:otherwise>
												</c:choose>
											</td>
											<td width="20%" class="table_grey"><span>*</span>是否已经行政复议：</td>
											<td colspan="2">
												<c:choose>
													<c:when test="${guestbook.ext.administrativeReconsideration}">
														<input type="radio" id="administrativeReconsideration" name="administrativeReconsideration" value="1" class="sex" checked>是 
														<input type="radio" id="administrativeReconsideration" name="administrativeReconsideration" value="0" class="sex">否
													</c:when>
													<c:otherwise>
														<input type="radio" id="administrativeReconsideration" name="administrativeReconsideration" value="1" class="sex">是 
														<input type="radio" id="administrativeReconsideration" name="administrativeReconsideration" value="0" class="sex" checked>否
													</c:otherwise>
												</c:choose>
											</td>
										</tr>
										<tr>
											<td width="20%" class="table_grey"><span>*</span>是否允许公开：</td>
											<td colspan="2">
												<c:choose>
													<c:when test="${guestbook.ext.publicity}">
														<input type="radio" id="publicity" name="publicity" value="1" class="sex" checked>是 
														<input type="radio" id="publicity" name="publicity" value="0" class="sex">否
													</c:when>
													<c:otherwise>
														<input type="radio" id="publicity" name="publicity" value="1" class="sex">是 
														<input type="radio" id="publicity" name="publicity" value="0" class="sex" checked>否
													</c:otherwise>
												</c:choose>
											</td>
												...
												<div class="sj_sc table_bordernone">
													...
													<div id="fileDocName">
														<c:forEach items="${guestbook.attachments}" var="list">
															<c:if test="${list.type==1}">${list.name}　</c:if>
														</c:forEach>
													</div>
													<div class="clear"></div>
												</div>
											...
											<td colspan="4">
												<div class="sj_sc table_bordernone">
													...
													<div id="fileMediaName">
														<c:forEach items="${guestbook.attachments}" var="list">
															<c:if test="${list.type==2}">${list.name}　</c:if>
														</c:forEach>
													</div>
													<div class="clear"></div>
												</div>
											...
											<td colspan="4">
												<div class="sj_sc table_bordernone">
													...
													<div id="fileVideoName">
														<c:forEach items="${guestbook.attachments}" var="list">
															<c:if test="${list.type==3}">${list.name}　</c:if>
														</c:forEach>
													</div>
													<div class="clear"></div>
												</div>
											...
											<td colspan="4">
												<div class="sj_sc table_bordernone">
													...
													<div id="fileImageName">
														<c:forEach items="${guestbook.attachments}" var="list">
															<c:if test="${list.type==4}">${list.name}　</c:if>
														</c:forEach>
													</div>
													<div class="clear"></div>
												</div>
										...
										<tr>
											...
											<td colspan="4"><textarea id="appeals" name="appeals">${guestbook.appeals}</textarea></td>
										</tr>
										<tr>
											<td width="20%" class="table_grey"><span>*</span>初步处理意见：</td>
											<td colspan="4"><textarea id="preliminaryOpinion" name="preliminaryOpinion">${guestbook.preliminaryOpinion}</textarea></td>
										</tr>
										<tr>
								...
								<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_one">
									<tbody>
										<tr>
											...
											<td width="30%" colspan="2"><input class="input_color" id="reportedName" name="reportedName" value="${guestbook.ext.reportedName}" type="text" placeholder="请输入被举报人 " maxlength="10" /></td>
											<td width="20%" class="table_grey"><span>*</span>单　　位：</td>
											<td width="30%" colspan="2"><input class="input_color" type="text" id="reportedCompany" name="reportedCompany" value="${guestbook.ext.reportedCompany}" placeholder="请输入单位" maxlength="50" /></td>
										</tr>
										<tr>
											<td width="20%" class="table_grey"><span>*</span>职　　务：</td>
											<td width="30%" colspan="2"><input class="input_color" id="reportedDuties" name="reportedDuties" value="${guestbook.ext.reportedDuties}" type="text" placeholder="请输入职务" maxlength="30" /></td>
											<td class="table_grey"><span>*</span>级　　别：</td>
											<td colspan="2">
												<select id="reportedLevel" name="reportedLevel">
													<option value=""></option>
													<c:forEach items="${levelList }" var="obj" varStatus="status">
														<option value="${obj.name }" ${guestbook.ext.reportedLevel==obj.name?'selected':''}>${obj.name }</option>
													</c:forEach>
												</select>
											</td>
										</tr>
