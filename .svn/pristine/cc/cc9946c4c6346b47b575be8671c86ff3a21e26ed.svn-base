(function ($, undefined) {
	...
	function Globals() {
		this.settings = { "show_on" : 'mouseover' };
	}
	...
	function Spot(x, y) {
		...
		this.y = y;
		this.content = '';		
		this.html = '<div class="hb-spot hb-spot-object" id="hb-spot-' + this.id + '"><div class="hb-tooltip-wrap"></div></div>';
		this.css = '';
		this.root = '';
		this.width = 30;
		this.height = 30;
	...
	function dynamic_events() {
		...
		$('.hb-move-handle').on('mouseover.hb', function() {
			show_tooltip('move');//在这里修改鼠标移动文本框里的字
		});
	...
	function result() {
		...
		$('#hb-live-preview').find('.hs-wrap').hotspot({ 'show_on' : globals.settings['show_on'] });
	}
}(jQuery));